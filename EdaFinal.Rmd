...
Separaremos el archivo en dos subconjuntos: datos de entrenamiento y datos de validación. El primero de los grupos se llamará *housesTrain* y el segundo *housesValidation*.
...
housesTrain$car_cat <-cut(imputationsCarFinal$Car, breaks = c(0,2,9), labels = c("Hasta_1_plaza", "2_o_mas_plazas"), include.lowest = TRUE, right = FALSE)
# Estandarización de variables
num_vars <- c('Distance', 'sqrt_distance', 'log_landsize', 'Lattitude', 'Longtitude', 'Rooms')
...
```{r}
#SI SE EJECUTA ESTO LUEGO EL GLMNET NO FUNCIONA, HAY QUE VERLO
housesTrainFinal$Regionname = factor(housesTrainFinal$Regionname, levels=c('Southern Metropolitan', 'Northern Metropolitan', 'Western Metropolitan', 'Eastern Metropolitan', 'Eastern Victoria', 'Northern Victoria', 'Western Victoria'))
levels(housesTrainFinal$Regionname) <- c('Southern_Metropolitan', 'Northern_Metropolitan', 'Western_Metropolitan', 'Eastern_Metropolitan', 'Eastern_Victoria', 'Northern_Victoria', 'Western_Victoria')
...
```{r}
housesTrain %>% select(sqrt_distance, log_landsize, log_price, sellYear) %>% ggpairs(columns=1:3, ggplot2::aes(colour=sellYear, alpha=0.05))
```
...
```{r}
lm_model_13 = lm(log_price ~ sqrt_distance + Lattitude + Longtitude + rooms_catMedianas + rooms_catGrandes + car_cat2 o más plazas + RegionnameNorthern Metropolitan +
                   RegionnameSouthern Metropolitan + Typet + Typeu)
```
