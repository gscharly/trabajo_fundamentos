...
year_categoriesImp <- imputationsYearBuilt %>% mutate(year_cat2 = cut2(YearBuilt, g=4))
...
train$year_cat_imputation <- year_categoriesImp$year_cat2
head(train)
```
...
```{r}
model_yearcat <- lm(Price~sqrt_distance + Lattitude + Longtitude + rooms_cat + Type + year_cat_imputation, data = train)
