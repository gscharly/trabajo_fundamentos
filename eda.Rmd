...
dfDivision <- data.frame(
  Subconjuntos = c('Test + Validation', 'Test'),
...
train$year_cat_imputation <- year_categoriesImp$year_cat2 #añado la nueva imputada
train$YearBuilt <- year_categoriesImp$YearBuilt
head(train)
```
...
```{r}
model_year <- lm(Price~sqrt_distance + Lattitude + Longtitude + Type + rooms_cat + Regionname, data = train)
summary(model_year)
```
...
```{r}
model_yearcat <- lm(Price~sqrt_distance + Lattitude + Longtitude + rooms_cat + Type + year_cat_imputation + Regionname, data = train)
