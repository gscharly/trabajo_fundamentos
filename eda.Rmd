...
En resumen:
- Para la variable *BuildingArea*, el 47% de los valores (5145 de 10865) son NaNs, y 0.11% son 0. En total, tendríamos 48.11% de valores informados.
...
```
```{r}
housesTrainImputed <- housesTrain %>%
  filter((YearBuilt > 1200 | is.na(YearBuilt)) & (BuildingArea < 2500 | is.na(BuildingArea))) %>%
...
```{r}
housesTrain[housesTrain$Landsize==0, ]$Landsize <- NA
