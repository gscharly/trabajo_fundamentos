...
*Objetivo general*
* El objetivo es la predicción del precio de venta de una vivienda en la ciudad de Melbourne. Se trata pues de un problema de regresión.
...
* Transformación de variables para su uso adecuado en un modelo de regresión lineal.
...
En el dataset contamos con ocho variables categóricas: *Suburb*, *Address*, *Type*, *Method*, *SellerG*, *Date*, *CouncilArea* y *Regionname*. A continuación se muestra información de cada una de ellas. 
...
Además de esto, respecto a la variable *Date* nos parece más relevante mostrar las frecuencias por meses del año por lo que la separamos en tres variables distintas: *day*, *month* y *year*.
...
Distribución de precio en función de la región. Western Victoria parece que tiene unos precios más bajos, mientras que Southern Metropolitan y Eastern Metropolitan parece que tienen mayores precios. Hay que tener en cuenta que hay muy pocas muestras de las regiones de Victoria (son zonas más alejadas, referirse al mapa)
...
```
<br>
¿Hay relación con otras variables?
```{r}
housesTrain %>% mutate(rooms_cat = cut(housesTrain$Rooms, breaks = c(1,3,4,10), labels = c("De 1 a 2", "3", "De 4 a 10"), include.lowest = TRUE, right = FALSE), 
                       log_price = log10(Price)) %>%
  select(Distance, log_price, rooms_cat) %>%
  ggpairs(columns=1:3, ggplot2::aes(colour=rooms_cat, alpha=0.1))
```
```{r}
housesTrain %>% mutate(rooms_cat = cut(housesTrain$Rooms, breaks = c(1,3,4,10), labels = c("De 1 a 2", "3", "De 4 a 10"), include.lowest = TRUE, right = FALSE)) %>%
  select(Distance, Price, rooms_cat) %>%
  ggpairs(columns=1:3, ggplot2::aes(colour=rooms_cat, alpha=0.1))
```
...
```{r}
housesTrainFinal %>% select(log_landsize, log_price, Regionname) %>% ggpairs(columns=1:2, ggplot2::aes(colour=Regionname, alpha=0.1))
```
