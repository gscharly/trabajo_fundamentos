...
```{r}
housesTrain$sellYear <- housesTrain %>% separate(Date, c('day','month', 'year'), sep = '/') %>% select(year)
#housesTrain$sellYear <- sapply(housesTrain$sellYear, as.numeric)
#housesTrain$sellYear <- factor(housesTrain$sellYear)
```
...
```{r}
housesTrain %>% select(Rooms, Car) %>% na.omit() %>% ggplot(aes(x=Rooms, y=Car)) + geom_point() + geom_smooth(method='lm')
housesTrain %>% select(Distance, Car) %>% na.omit() %>% ggplot(aes(x=Distance, y=Car)) + geom_point() + geom_smooth(method='lm')
housesTrain %>% select(Longtitude, Car) %>% na.omit() %>% ggplot(aes(x=Longtitude, y=Car)) + geom_point() + geom_smooth(method='lm')
```
...
housesTrain$rooms_cat <- housesTrain %>% mutate(rooms_cat = cut(housesTrain$Rooms, breaks = c(1,3,4,10), labels = c("Pequeñas", "Medianas", "Grandes"), include.lowest = TRUE, right = FALSE)) %>% select(rooms_cat)
#housesTrainFinal$rooms_cat <- factor(housesTrainFinal$rooms_cat)

housesTrain$year_built_cat <- housesTrain %>%
  mutate(year_cat = addNA(cut2(YearBuilt, g=2))) %>% select(year_cat)
#housesTrainFinal$year_built_cat <- factor(housesTrainFinal$year_built_cat)
```
...
```
```{r}
housesTrain %>% select(Distance, Price) %>% ggplot(aes(x=Distance, y=Price)) + geom_point()
```


```{r}
housesTrainFinal %>% select(Distance, Price, rooms_cat) %>% ggpairs(columns=1:2, ggplot2::aes(colour=rooms_cat))
```
