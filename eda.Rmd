...
```{r yearfilterfunciton, echo = F, message=F, warning=F}
year_filter <- function(dataset){ dataset <- dataset %>% filter(dataset$YearBuilt > 1803 | is.na(YearBuilt)) }  

housesTrain <- year_filter(housesTrain)
```
...
Esta variable toma el valor de 1196 en una de las observaciones. El primer asentamiento en el área que actualmente se corresponde con la ciudad de Melbourne data de 1803, por lo tanto este dato es evidentemente erróneo. Hemos establecido una regla de filtrado a nuestro dataset de tal manera que si entra alguna observación en la cual la variable *Yearbuilt* sea anterior al año 1835 no será descartada por tratarse de un dato erróneo.

Una vez eliminado ese registro, se dispone de 6602 valores informados.
...
Hemos decidido tratar los valores nulos de esta variable como una categoría adicional que llamaremos "unknown". 
...
imputations <- housesTrain %>%
  select(BuildingArea, Landsize, Car) %>%
  VIM::kNN()
```
...
housesTrain <- housesTrain %>%
  mutate(year_cat = cut2(YearBuilt, g=5))

levels(housesTrain$year_cat) <- c(levels(housesTrain$year_cat), 'Unknown')

housesTrain$year_cat[which(is.na(housesTrain$YearBuilt))] <- 'Unknown'
