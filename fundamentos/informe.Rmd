...
Para este ejercicio, se eligen al azar varias casas del dataset, y se proponen cambios en las variables de entrada del modelo para que el precio de venta se incremente en un 10%. Al tener nuestro modelo solo 2 variables cuantitativas (las coordenadas), se calculará también el incremento que supondría un cambio de categoría en algunas de las variables.

Al ser las variables cuantitativas de este modelo unicamente las coordenadas, no tendría mucho sentido hablar de desplazamientos de este-oeste y de norte-sur por separado. Hay que tener también en cuenta que si las coordenadas varían, también lo pueden hacer algunas categorías, como *Regionname* o *sell_rate*. Además, existe el problema obvio de que si los cambios son grandes las coordenadas se pueden salir de la ciudad, o acabar en el mar.
...
```{r}
i=2
house <- housesTrain[i,] %>% select(Lattitude, Longtitude, Type, Rooms, Regionname, Bathroom, YearBuilt, Propertycount, Method, Car, log_price, Price)
house

housePruebaTrain <- train[i,] %>% select(Lattitude, Longtitude, Type, rooms_cat, Regionname, bath_cat, year_built_cat, sell_rate_cat, Method, car_cat, log_price)
housePruebaTrain
```
...
```{r}
prediction_actual <- 10^predictions[i]
prediction_final <- prediction_actual*1.1
log_prediction_final <- log10(prediction_final)
```



```{r diffLattitude}
prediction_without_var <- predictions[i] - housePruebaTrain$Lattitude*coefs['Lattitude']
new_var_scaled <- (log_prediction_final - prediction_without_var)/coefs['Lattitude']
...
```{r diffLongtitude}
prediction_without_var <- predictions[i] - housePruebaTrain$Longtitude*coefs['Longtitude']
new_var_scaled <- (log_prediction_final - prediction_without_var)/coefs['Longtitude']
...
increment <- calculate_increment(predictions[i], predictions[i] + coefs['rooms_catMedianas'])
...
increment <- calculate_increment(predictions[i], predictions[i] + coefs['rooms_catGrandes'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
increment <- calculate_increment(predictions[i], predictions[i] + coefs['bath_catMuchos_baños'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
increment <- calculate_increment(predictions[i], predictions[i] - coefs['year_built_catModerna'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
```{r}
# Pasar de PI a S
increment <- calculate_increment(predictions[i], predictions[i] - coefs['MethodPI'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
```{r}
# Pasar de PI a S
increment <- calculate_increment(predictions[i], predictions[i] + coefs['car_catMuchas_plazas'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
Inicialmente se habían probado múltiples modelos uitlizando la variable Price (sin transformar) como target. Estos modelos finalmente fueron descartados debido a los malos resultados obtenidos. Sin embargo, nos ha parecido interesante reflejar en este anexo el proceso que hemos ido siguiendo para conseguir los resultados finales que se han mostrado anteriormente en este informe.
