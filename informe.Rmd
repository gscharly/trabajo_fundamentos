...
#Suburb, Address, Postcode, YearBuilt, Lattitude, Longtitude
...
tabla_frecuencias <- function(df, columna1, columna2 = 'Frecuencia'){
  ...
  if(length(df) > 11){
    knitr::kable(df, col.names = c(columna1, columna2)) %>%
      kable_styling(bootstrap_options = 'hover', position = 'center',full_width = T) %>% scroll_box(width = "100%", height = "350px")
...
`r for (var in colnames((VarCualitativas)[c(1,3,4,7,8,9,10)])){print(tabla_frecuencias(columna1 = var, df = data.frame(table(VarCualitativas[var]))))}`
...
Hacemos además una rápida comprobación de valores informados. Vemos que hay un outlier en el año 1196. Se establece que cualquier casa que tenga *YearBuilt* por debajo del año de fundación de la ciudad será eliminada.
...
A priori, parecería lógico pensar que las casas de tipo u son las más baratas, y las h más caras. Lo comprobamos.
