...
Para la variable objetivo *Price*, observamos que tiene valores muy extremos, lo que hace que la distribución quede muy sesgada a la izquierda. La transformación que parece más indicada para esta variable es de tipo logaritmo, ya que, además, no presenta valores iguales a 0.
...
```{r normalidadPrice}
housesTrain %>% ggplot(aes(sample=log_price)) + stat_qq() + stat_qq_line() + labs(title='qq-plot log_price')
...
Siguiendo con nuestro objetivo principal, es decir predecir el precio de una vivienda en base a sus características, vamos a analizar la relación de las variables del dataset con respecto a la variable *Price*. Siguiendo con la transformación realizada sobre esta variable, se tomará el logaritmo del precio para su comparación con el resto de variables, por presentar una distribución más cercana a una distribución normal.
...
Empezaremos examinando la variable logaritmo de Price en función de las variables categóricas que tenemos. 
...
housesTrain %>% 
  ...
  geom_density() + scale_colour_manual(values=palette34)
...
Como comprobación, se compara la variable numérica *Rooms* con *Type*, para ver cuantas habitaciones tienen los distintos tipos de casa y así comprender mejor qué implica la variable *Type*. Podemos observar que las casas de tipo u son las que menos habitaciones tienen (1, 2 o 3), seguidas de las casas tipo t (2, 3 y 4) y terminando con las h, que tienen todos los posibles valores de habitaciones.

```{r roomsType}
housesTrain %>% ggplot(aes(x=Rooms, fill=Type)) + geom_histogram() + scale_fill_manual(values=palette34)
```
...
p1 <- housesTrain %>% ggplot(aes(x=log_price, color=may_have_water)) + geom_density() + scale_color_manual(values = palette34)

p2 <- housesTrain %>% ggplot(aes(y=log_price, fill=may_have_water)) + geom_boxplot() + scale_fill_manual(values=palette34)
...
A continuación, examinaremos las distintas variables numéricas, y su relación con la que será nuestra variable objetivo *Price*, tomando su logaritmo.
...
- El año de construcción (*YearBuilt*) tiene una correlación negativa con el precio, es decir, las casas nuevas pueden ser más baratas que las antiguas. De nuevo, tenemos el mismo problema con la distribución de esta variable, además de su gran cantidad de valores faltantes.
...
En ambas categorizaciones se observa cómo son más numerosas las viviendas con 1, 2 o 3 habitaciones y cómo la cola derecha la la distribución es mayor cuanto mayor es el número de habitaciones. Nos centraremos en la división en 3 grupos, que se corresponderían con casas Pequeñas, Medianas y Grandes.
...
p <- housesTrain %>%
  ...
  ggpairs(ggplot2::aes(colour=sellYear, alpha=0.1))
...
```{r message=FALSE}
p <- housesTrain %>% select(sqrt_distance, Lattitude, Longtitude, sellYear) %>% ggpairs(columns=1:3, ggplot2::aes(colour=sellYear, alpha=0.1))
...
```{r message=FALSE}
housesTrain %>% ggplot(aes(y=Price, color=sellMonth)) + geom_boxplot() + scale_colour_manual(values=palette34)
```
...
housesTrain %>% mutate(property_count_cat = cut2(Propertycount, g=3)) %>% ggplot(aes(x=log_price, color=property_count_cat)) + geom_density() + scale_colour_manual(values=palette34)
```
...
housesTrain %>% mutate(property_count_cat = cut2(sell_rate_suburb, g=2)) %>%  ggplot(aes(x=log_price, color=property_count_cat)) + geom_density() + scale_colour_manual(values=palette34)
```
...
```{r message=FALSE}
p <- housesTrainFinal %>% select(sqrt_distance, log_landsize, log_price, rooms_cat) %>% ggpairs(columns=1:3, ggplot2::aes(colour=rooms_cat, alpha=0.1))
...
```{r}
p <- housesTrainFinal %>% select(sqrt_distance, log_landsize, log_price, car_cat) %>% ggpairs(columns=1:3, ggplot2::aes(colour=car_cat, alpha=0.1))
...
Atendiendo al año de construcción de la vivienda, las antiguas está ligeramente más próximas al centro, más caras y más grandes, pero esta variable no discrimina de forma marcada. 

```{r}
p <- housesTrainFinal %>% select(sqrt_distance, log_landsize, log_price, year_built_cat) %>% ggpairs(columns=1:3, ggplot2::aes(colour=year_built_cat, alpha=0.1))

for(i in 1:p$nrow) {
  for(j in 1:p$ncol){
    p[i,j] <- p[i,j] + 
        scale_fill_manual(values=c(palette34[5], palette34[4], palette34[3])) +
        scale_color_manual(values=c(palette34[5], palette34[4], palette34[3])) 
  }
}
...
Las viviendas tipo u se concentran en zonas más próximas al centro, tienen más varianza en cuanto al tamaño y cuentan con precios de venta más económicos. Las viviendas tipo h y t se encuentran más alejadas del centro. Además las de tipo h tienden a ser más caras y tener, en su mayoría, tamaños medio-grandes, mientras que las de tipo t presentan tamños más pequeños y precios muy concentrados en valores medios.

```{r}
p <- housesTrainFinal %>% select(sqrt_distance, log_landsize, log_price, Type) %>% ggpairs(columns=1:3, ggplot2::aes(colour=Type, alpha=0.1))

for(i in 1:p$nrow) {
  for(j in 1:p$ncol){
    p[i,j] <- p[i,j] + 
        scale_fill_manual(values=c(palette34[6], palette34[4], palette34[8])) +
        scale_color_manual(values=c(palette34[6], palette34[4], palette34[8])) 
  }
}
...
Por último, respecto al método de venta se observa que no discrimina mucho respecto a las otras variables. Las viviendas vendidas con una oferta del vendedor (VB) está ligeramente más cera del centro y las vendidas en una subasta ligeramente más apartadas de este. En cuanto al tamaño, las distribuciones también están bastante superpuestas, pero si se ve cómo las VB y las SP tienden a tener mañor varianza. Las vendidas con oferta (VB) también presentan mayor varianza respecto al precio y , como es esperable, las vendidas mediante subasta presentan precios más concentrados en valores medios. 

```{r}
p <- housesTrainFinal %>% select(sqrt_distance, log_landsize, log_price, Method) %>% ggpairs(columns=1:3, ggplot2::aes(colour=Method, alpha=0.1))
...
Anteriormente hemos preseleccionado ciertas variables después de la limpieza y análisis inicial de nuestro conjunto de datos. En base al exploratorio realizado, hemos visto que no hay ninguna correlación lineal evidente entre las variables cuantitativas y la variable objetivo. Por tanto, vamos a aplicar diversos métodos de selección de variables para encontrar la mejor combinación posible. Actualmente disponemos de un total de 13 variables explicativas, de las cuales tan solo 4 son cuantitativas. Hay que añadir además que, si se codifican las variables categóricas como dummie, el modelo podría llegar a tener 24 variables.
...
Inicialmente se probó a utilizar la variable *Price* directamente como target. Sin embargo, en base a lo visto en el EDA acerca de la transformación conveniente de esta variable, el modelo de regresión lineal múltiple logra mejores resultados si se utiliza *log_price* como variable objetivo. Por tanto, solo se muestran las técnicas de selección de variables aplicadas a modelos con esta variable como objetivo.
...
reg_sum
```
Para evaluar los resultados obtenidos, se ha observado el coeficiente R2 ajustado asociado a cada modelo:

```{r}
reg_sum$adjr2
which.max(reg_sum$adjr2)
```
A juzgar por los resultados, el mejor modelo es el que consta de 23 variables. 

Otra manera más gráfica de ver de cuántos predictores consta el mejor modelo posible sería a través del siguiente gráfico, donde además del R2 ajustado se muestran otras estimaciones de la bondad de ajuste de los modelos:
...
En la anterior gráfica se puede observar que, aunque el modelo con el mayor R2 ajustado se consigue con 23 predictores (como ya habíamos visto), a partir de 17 predictores aproximadamente la mejoría es inapreciable. Como siempre va a ser mejor un modelo más explicable.

```{r}
reg_sum$adjr2[23]
reg_sum$adjr2[17]
```
Para el modelo de 17 predictores, los coeficientes son los siguientes:

```{r}
coef(object = best_subsets_models, id = 17)
```
...
p
```
De nuevo se cumple que el modelo que ofrece un mayor R^2 ajustado es el que utiliza 23 predictores. Ahora se necesitaría mínimo unos 19 predictores para alcanzar un valor similar.
...
Las conclusiones que se extraen son muy parecidas a las anteriores.

```{r}
reg_forward_sum$adjr2[19]
coef(object = best_forward_models, id = 19)
```
Se puede concluir, que las 3 técnicas de selección automática de variables indican que el número óptimo de predictores es alrededor de 19. Además, según estas técnicas, las variables más significativas para el modelo serían:

* Distancia al centro.
* Longitud.
* Latitud.
* Tipo de casa según el número de habitaciones.
* Tipo de casa según el número de plazas de parking.
* Tipo de casa según el número de baños.
* Tipo de vivienda.
* Región.

Cabe destacar que el tamaño de la parcela (*log_landsize*) parece que no aporta mucho al modelo según estos métodos.
...
En esta misma gráfica, podemos ver que se podría obtener un modelo con 21 predictores aproximadamente, cuyo error sería muy similar al del mejor modelo (que utiliza 24, la variable eliminada sería la categoría de año de construcción desconocido). Con este método 5 variables asociadas a categorías se eliminarían.
...
Se obtiene un MSE de 0.01633 con 21 variables, y 0.01603 con 24.
...
```{r predictLasso}
preds_lasso <- predict(out_eleven,x)
residuals_lasso <- y - preds_lasso
lillie.test(residuals_lasso)
qplot(residuals_lasso, geom = "histogram",
     bins = 30) +
...
ridge_coef <- predict(out_ridge,type="coefficients")[1:26,]
sort(abs(ridge_coef), decreasing = TRUE) 
```
En este caso, la información más valiosa que nos aporta Ridge es que le otorga bastante importancia al tipo de vivienda. Con ambas regularizaciones se ha visto que la variable cuantitativa *log_landsize* obtiene un coeficiente muy cercano a 0.

Con Ridge, se obtiene un MSE de 0.0165 con 25 variables.
...
qplot(residuals_ridge, geom = "histogram",
     bins = 30) +
     labs(title = "Histograma de residuos",x = "residual")
...
Para compensar ambos métodos de regularización (Lasso puede llegar a "sobreregularizar" el modelo), se puede utilizar la técnica Elastic Net, mediante la cual un parámetro *alpha* pondera el peso de cada una de las dos regularizaciones sobre los datos. 

Utilizando 10-fold cross validation, y eligiendo el lambda.1se, el mejor resultado se obtiene con un *alpha*=0.45. El modelo constaría de 22 variables (se eliminan variables que ya hemos visto anteriormente: año desconocido, región de Victoria Norte y método SA) y tendría un MSE de 0.01625.
...
search <- foreach(i = a, .combine = rbind) %do% {
  ...
  data.frame(cvm = cv$cvm[cv$lambda == cv$lambda.1se], lambda.1se = cv$lambda.1se, alpha = i, type.measure='mse')
}
cve <- search[search$cvm == min(search$cvm), ]
mde <- glmnet(x, y, lambda = cve$lambda.1se, alpha = cve$alpha)
cve
mde
coef(mde)
```
...
```{r predictElastic}
preds_e <- predict(mde,x)
residuals_e <- y - preds_e
lillie.test(residuals_e)
qplot(residuals_e, geom = "histogram",
     bins = 30) +
     labs(title = "Histograma de residuos",x = "residual")
rsq_e <- cor(y, preds_e)^2
sprintf("R2 = %f", rsq_e)
```
...
Como se ha comentado antes, después de probar modelos con target *Price*, se decidió utilizar la variable *log_price* como objetivo. El mejor resultado obtenido con esta primera forma fue de R2=0.54.

Para el primer modelo con el nuevo target, se utilizan todas las variables con las que hemos estado trabajando, ya que con las técnicas de selección automática se ha visto que las variables que se pueden obviar son en su mayoría categorías de variables cualitativas.

Observaciones:

* Se obtiene un R2 = 0.6948 con un MSE = 0.0159.
* Las variables *RegionnameNorthern_Victoria*, *RegionnameWestern_Victoria*, *year_built_catDesconocido* y *MethodSA* obtienen un p-valor elevado, con lo que no se podría rechazar la hipótesis nula de que sus coeficientes son 0. Esto ya lo hemos visto en las técnicas de selección de variables.
* Los residuos no siguen una distribución normal. El test de normalidad tiene un p-valor muy pequeño, con lo que se podría rechazar la hipótesis de normalidad. Además, en la distribución de residuos vs predicciones aparecen outliers (debería ser uniforme entre -1 y 1), y el histograma de los residuos no llega a ser del todo normal.

```{r}
model1 <- lm(log_price~ sqrt_distance + log_landsize + Lattitude + Longtitude + Type + rooms_cat + Regionname + bath_cat + year_built_cat + sell_rate_cat + Method + car_cat, data = train)
summary(model1)
check_residuals_normality(model1)
qplot(model1$residuals, geom = "histogram",
     bins = 30) +
     labs(title = "Histograma de residuos",x = "residual")
mse_model <- calculate_mse(model1)
sprintf("MSE: %f", mse_model)
```

Las razones por las que los errores no son normales pueden ser:

* Relación no lineal entre entradas y salidas. Ya hemos visto que nuestras variables predictoras no tienen una correlación muy alta con la salida.
* Distribuciones de valores de entrada. Ya se ha visto que variables como *sqrt_distance* o *log_landsize* no son del todo normales a pesar de las transformaciones aplicadas.

## Modelo 2
Para comprobar si las distribuciones de las entradas pueden afectar a la normalidad de los residuos, probamos otro modelo en el que no se incluye la variable *sqrt_distance*.

Observaciones:

* Como era de esperar, obtenemos un R2 menor que antes (la variable *sqrt_distance* aportaba información relevante al modelo) con un MSE = 0.0215.
* Ahora, la variable *log_landsize* tiene un p-valor elevado. Esto, unido a lo que se ha visto en las técnicas de selección de variables, parece indicar que esta variable no aporta información lineal acerca del precio de la vivienda.
* Aunque el p-valor del test de normalidad de los residuos sea pequeño, es mucho mayor que antes. Esto unido a la gráfica de residuos vs predicciones, donde parece que la varianza aumenta con las predicciones (parte izq de la gráfica) pero en menor medida que antes y el histograma de los residuos, se puede establecer que los residuos siguen una distribución aproximadamente normal.

```{r}
model2 <- lm(log_price~log_landsize + Lattitude + Longtitude + Type + rooms_cat + Regionname + bath_cat + year_built_cat + sell_rate_cat + Method + car_cat, data = train)
summary(model2)
check_residuals_normality(model2)
qplot(model2$residuals, geom = "histogram",
     bins = 30) +
     labs(title = "Histograma de residuos",x = "residual")
mse_model2 <- calculate_mse(model2)
sprintf("MSE: %f", mse_model2)
```

Probamos además a quitar la variable *log_landsize* para comprobar lo mencionado anteriomente.

Se puede comprobar que efectivamente no hay efecto al quitar esa variable.

```{r}
model3 <- lm(log_price~Lattitude + Longtitude + Type + rooms_cat + Regionname + bath_cat + year_built_cat + sell_rate_cat + Method + car_cat, data = train)
summary(model3)
check_residuals_normality(model3)
qplot(model3$residuals, geom = "histogram",
     bins = 30) +
     labs(title = "Histograma de residuos",x = "residual")
mse_model3 <- calculate_mse(model3)
```


## Interacciones entre variables

Por último, se prueban interacciones entre variables con el fin de mejorar el modelo.
...
Las opciones que hemos visto que mejoran alguno de los modelos son:

__Distance y landsize__
...
En este caso, se combinan estas dos variables sin incluir la distancia ni el tamaño de la parcela.

```{r lat*long_no_distance}
model <- lm(log_price~Lattitude*Longtitude + Type + rooms_cat + Regionname + bath_cat + year_built_cat + sell_rate_cat + Method + car_cat, data = train)
summary(model)
check_residuals_normality(model)
qplot(model$residuals, geom = "histogram",
     bins = 30) +
...
__Regionname y rooms_cat__

También se ha probado a combinar variables cualitativas.
...
Tenemos dos modelos candidatos:

* Modelo 1: con un R2 mejor pero sin cumplir normalidad en los errores.
* Modelo 2: con un R2 peor pero los errores se acercan más a una distribución normal.

Probamos este último modelo, ya que las predicciones tendrán un intervalo de confianza más apropiado, y las predicciones del modelo se ajustarán más a la "realidad" (aunque fallará más).
...
model_metrics <- data.frame(error_metrics = c('R2','MSE'), value_train = c(summary(best_model)$r.squared, calculate_mse(best_model)), value_test = c(r2, rmse^2))

model_metrics
```

Se puede observar por el valor que toma R2 (muy parecido al de train), que nuestro modelo, a pesar de que no logra un gran acierto, no sufre ningún tipo de sobreajuste.
...
- El análisis nos ha mostrado que no hay una relación lineal evidente entre las variables predictoras y el precio de la vivienda. Esta es una de las razones principales por las que se considera que el modelo no da los mejores resultados, sobretodo al analizar los residuos.
...
- Identificar una variable/variables para utilizar el dataset en un problema de clasificación (tipo de casa? método de venta de casa?)
- Previsiblemente, un modelo como un árbol de decisión o ensembles de arboles (tipo Random Forest) aplicado al problema de predicción del precio ofrezca mejores resultados, al trabajar mejor con variables categóricas.
- Estudiar si otras variables como el código postal, barrio o vendedor pueden transformarse en menos categorías para ser aprovechable en modelos.

<br>

# Ejercicio extra: ¿Cómo aumentar el precio un 10%?

Para este ejercicio, se eligen varias casas del dataset, y se proponen cambios en las variables para que el precio de venta se incremente en un 10%. Al tener nuestro modelo solo 2 variables cuantitativas (las coordenadas), se calculará también el incremento que supondría un cambio de categoría en algunas de las variables.

Al ser las variables cuantitativas de este modelo en especial las coordenadas, no tendría mucho sentido hablar de desplazamientos de este-oeste y de norte-sur por separado. Hay que tener en cuenta que si las coordenadas varían, también lo pueden hacer algunas categorías, como *Regionname* o *sell_rate*. Además, está el problema obvio de que si los cambios son grandes las coordenadas se pueden salir de la ciudad, o acabar en el mar.


```{r}
coefs <- model3$coefficients
predictions <- model.matrix(model3) %*% coefs
coefs
```
## Casa 1

Se muestra la información original y transformada de la casa:
```{r}
house <- housesTrain[2,] %>% select(Lattitude, Longtitude, Type, Rooms, Regionname, Bathroom, YearBuilt, Propertycount, Method, Car, log_price, Price)
house

housePruebaTrain <- train[2,] %>% select(Lattitude, Longtitude, Type, rooms_cat, Regionname, bath_cat, year_built_cat, sell_rate_cat, Method, car_cat, log_price)
housePruebaTrain
```


Para esta casa, se propone un cambio en la latitud, manteniendo constantes los demás parámetros.

```{r}
price_actual <- house$Price
price_final <- price_actual*1.1
log_price_final <- log10(price_final)
residual <- model3$residuals[2]
```


```{r diffLattitude}
prediction_without_var <- predictions[2] - housePruebaTrain$Lattitude*coefs['Lattitude']
new_var_scaled <- (log_price_final - prediction_without_var - residual)/coefs['Lattitude']
new_var <- new_var_scaled*var(housesTrain$Lattitude) + mean(housesTrain$Lattitude)
sprintf("Latitud original: %f", house$Lattitude)
sprintf("Latitud modificada: %f", new_var)
```

Esto significa que para aumentar el precio un 10%, la vivienda debería estar unos grados más al sur, como se muestra en el mapa.
```{r echo=FALSE, message=FALSE}
melbourne_map <- get_map(location = "melbourne", zoom = 13)
```


```{r message=FALSE}
changes_lat <- data.frame(Lattitude=c(house$Lattitude, new_var), Longtitude=c(house$Longtitude, house$Longtitude), color=c(palette34[1], palette34[2]))
ggmap(melbourne_map) + geom_point(aes(x = changes_lat$Longtitude, y = changes_lat$Lattitude, color=changes_lat$color), data = changes_lat, alpha=0.5, size = 3) + theme(legend.position="none")
```
...
```{r}
house <- housesTrain[10,] %>% select(Lattitude, Longtitude, Type, Rooms, Regionname, Bathroom, YearBuilt, Propertycount, Method, Car, log_price, Price)
house

housePruebaTrain <- train[10,] %>% select(Lattitude, Longtitude, Type, rooms_cat, Regionname, bath_cat, year_built_cat, sell_rate_cat, Method, car_cat, log_price)
housePruebaTrain
```

Para esta casa, se propone un cambio en la longitud.

```{r}
price_actual <- house$Price
price_final <- price_actual*1.1
log_price_final <- log10(price_final)
residual <- model3$residuals[10]
```

```{r diffLongtitude}
prediction_without_var <- predictions[10] - housePruebaTrain$Longtitude*coefs['Longtitude']
new_var_scaled <- (log_price_final - prediction_without_var - residual)/coefs['Longtitude']
new_var <- new_var_scaled*var(housesTrain$Longtitude) + mean(housesTrain$Longtitude)
sprintf("Longitud original: %f", house$Longtitude)
sprintf("Longitud modificada: %f",new_var)
```

Esto significa que para aumentar el precio un 10%, la vivienda debería estar unos grados más al este, como se muestra en el mapa.

```{r}
changes_lon <- data.frame(Lattitude=c(house$Lattitude, house$Lattitude), Longtitude=c(house$Longtitude, new_var), color=c(palette34[1], palette34[2]))
ggmap(melbourne_map) + geom_point(aes(x = changes_lon$Longtitude, y = changes_lon$Lattitude, color=changes_lat$color), data = changes_lat, alpha=0.5, size = 3) + theme(legend.position="none")
```
...
```{r}
house <- housesTrain[50,] %>% select(Lattitude, Longtitude, Type, Rooms, Regionname, Bathroom, YearBuilt, Propertycount, Method, Car, log_price, Price)
house

housePruebaTrain <- train[50,] %>% select(Lattitude, Longtitude, Type, rooms_cat, Regionname, bath_cat, year_built_cat, sell_rate_cat, Method, car_cat, log_price)
housePruebaTrain
```

Para esta casa, se van a estudiar cambios en algunas variables categóricas, para ver cual sería el incremento en el precio.

```{r echo=FALSE}
calculate_increment <- function(prediction, prediction_final) {
  prediction_final_price <- 10^prediction_final
  prediction_original_price <- 10^prediction
  incremento <- (prediction_final_price - prediction_original_price)/prediction_original_price*100
  return(incremento)
}
```

### Habitaciones
Esta casa tiene 2 habitaciones. ¿Qué pasaría si se construyera otra habitación (pasar a categoría Mediana)? Esto se traduciría en sumar el coeficiente de la categoría Mediana, ya que la categoría Pequeña es el valor de referencia en el factor.

```{r}
# Pasar de pequeña a mediana
increment <- calculate_increment(predictions[50], predictions[50] + coefs['rooms_catMedianas'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
¿Y si se construyen más habitaciones hasta llegar a 4 (o más)?

```{r}
# Pasar de pequeña a grande
increment <- calculate_increment(predictions[50], predictions[50] + coefs['rooms_catGrandes'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```

### Baños
Esta casa tiene 1 baño. ¿Y si se construyen más? De nuevo, la categoría de referencia es Pocos baños, por lo que bastaría con sumar el coeficiente de la categoría nueva.
```{r}
# Pasar de pocos a muchos baños
increment <- calculate_increment(predictions[50], predictions[50] + coefs['bath_catMuchos_baños'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```

### Año de construcción
Esta casa se construyó en 1975, y hemos visto que las casas antiguas tenían un precio más elevado. ¿Cuál sería el incremento del precio si la casa se hubiera construido antes del año 1971? En este caso, se debe restar el coeficiente de la categoría Moderna, puesto que la categoría Antigua es la referencia.

```{r}
# Pasar de moderna a antigua
increment <- calculate_increment(predictions[50], predictions[50] - coefs['year_built_catModerna'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
```{r}
house <- housesTrain[22,] %>% select(Lattitude, Longtitude, Type, Rooms, Regionname, Bathroom, YearBuilt, Propertycount, Method, Car, log_price, Price)
house

housePruebaTrain <- train[22,] %>% select(Lattitude, Longtitude, Type, rooms_cat, Regionname, bath_cat, year_built_cat, sell_rate_cat, Method, car_cat, log_price)
housePruebaTrain
```

### Método de venta
¿Y si el método de venta hubiese sido venta normal (S) en vez de PI? La categoría de referencia de nuevo es S.

```{r}
# Pasar de PI a S
increment <- calculate_increment(predictions[22], predictions[22] - coefs['MethodPI'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```

### Car
La casa tiene pocas plazas (menos de 2). ¿Cuánto aumentaría el precio si se construyen más?
```{r}
# Pasar de PI a S
increment <- calculate_increment(predictions[22], predictions[22] + coefs['car_catMuchas_plazas'])
sprintf("El incremento en el precio es del %.2f%%", increment)
```
...
En cuanto a la normalidad de los residuos, podemos ver que en esta ocasión sí que se ajustan bastante más a una distribución normal:
