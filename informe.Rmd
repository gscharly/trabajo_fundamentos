...
houses$region_colours <- revalue(houses$Regionname, c("Southern Metropolitan" = "#1B9E77", "Western Metropolitan" = "#D95F02", "Northern Metropolitan" = "#7570B3",
...
ggmap(melbourne_map) + geom_point(aes(x = houses$Longtitude, y = houses$Lattitude), colour=houses$region_colours , data = houses, alpha=0.5, size = 0.75)
```
...
housesTrain %>%
  filter(Bedroom2!=20) %>%
  mutate(bed_cat = cut2(Bedroom2, g=4)) %>% select(bed_cat) %>%
  table()

housesTrain %>%
  filter(Bedroom2!=20) %>%
  mutate(bed_cat = cut2(Bedroom2, g=4))  %>%
...
housesTrain %>% mutate(property_count_cat = cut2(Propertycount, g=3)) %>% ggplot(aes(x=log_price, color=property_count_cat)) + geom_density()
```
...
housesTrain %>% mutate(property_count_cat = cut2(sell_rate_suburb, g=5)) %>% select(property_count_cat) %>% table() 
housesTrain %>% mutate(property_count_cat = cut2(sell_rate_suburb, g=5)) %>%  ggplot(aes(x=log_price, color=property_count_cat)) + geom_density()
```
