...
```
```{r warning=F}
ks.test(housesTrain$log_price, pnorm, mean(housesTrain$log_price), sd(housesTrain$log_price))
```
...
p1 <- housesTrain %>% select(Landsize) %>%
  ...
  geom_boxplot() + labs(y = 'log_landsize inicial')

p2 <- imputationsLogLandsize %>% select(log_landsize) %>%
  ggplot(aes(y=log_landsize)) +
  geom_boxplot() + labs(y = 'log_landsize imputado')
