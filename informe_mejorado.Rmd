...
#melbourne_map <- get_map(location = "melbourne", zoom = 11)
#houses$region_colours <- revalue(houses$Regionname, c("Southern Metropolitan" = "#1B9E77",
                                                               # "Western Metropolitan" = "#D95F02",
                                                           #"Northern Metropolitan" = "#7570B3",
                                                           #"Eastern Metropolitan" = "#E7298A",
                                                           #"South-Eastern Metropolitan" = "#66A61E",
                                                           #"Eastern Victoria" = "#A6761D", 
                                                           #"Northern Victoria" = "#1DA632",
                                                           #"Western Victoria" = "#1B849E"))
  
#ggmap(melbourne_map) + geom_point(aes(x = houses$Longtitude, y = houses$Lattitude), #colour=houses$region_colours , data = houses, alpha=0.5, size = 0.5)
```
...
Respecto a esta última variable, *YearBuilt*, vemos que hay un outlier que se corresponde con el año 1196 según la tabla describe. Este dato lo tomamos como un dato erróneo (faltante) y e establece que cualquier casa que tenga *YearBuilt* por debajo del año de fundación de la ciudad (1850) será eliminada.
...
Por último,observamos que son varias las distribuciones con una fuerte curtósis y asimetría (*Price*, *Landsize*, *BuildingArea*, *YearBuilt*) por lo que anticipamos que esas variables necesitarán un tratamiento especial como veremos más adelante.
...
Observando las ráficas podemos apreciar como las viviendas de tipo u (unit) y t townhouse tienen menor dispersión en el precio y son más baratas. Sin embargo, para las viviendas tipo h cuenta con precios más caros y la varianza es mayor, lo cual tiene sentido ya que este tipo engloba tipos de viviendas más diversos.
Presumiblemente, parece que el typo si discrima con respecto al precio de la vivienda.
...
Los tipos de métodos eran los siguientes:
...
La gráfica tomando el logaritmo permite una visualización mejor donde vemos precios con bastante bariavilidad para todos los métodos de venta excepto para SA. Estas viviendas se han vendido después de una subasta, lo cual puede explicar que el precio esté más regulado. Atendiendo a los Boxplots, se observan rangos intercuantiles parecidos para algunos métodos (por ejemplo: S y SA) y diferentes para otros (por ejemplo: SB y VB). En este sentido queda por ver si el método resulta útil en el modelo a la hora de diferenciar el precio.
...
Por el número de niveles que tiene esta variable, resulta dificil apreciar algunas diferencias con respecto al precio, pero si observamos diferencias en cuanto a la variabilidad del precio. También llama la atención algunos de los outliers, donde hay viviendas con precios muy altos y otras muy bajos.
...
En ambas categorizaciones se obserba cómo son más numerosas las viviendas con 1, 2 o 3 habitaciones y cómo la cola derecha la la distribución es mayor cuanto mayor es el número de habitaciones.

__Car y Price__

Los valores de *Car* se concentran en 1 o 2 por lo que, al igual que la anterior variable, es candidata a discretización.

Por las gráficas obtenidas tras la categorización, no parece que el número de plazas de garaje sea un dato discriminante respecto al precio.
...
housesTrain %>% 
  ...
  mutate(car_cat = cut(Car, breaks = c(0,2,9), labels = c("De 0 a 1", "De 2 a 9"), include.lowest = TRUE, right = FALSE)) %>%
  select(Price, car_cat) %>%
  ggplot(aes(y=Price, fill=car_cat)) +
  geom_boxplot() + scale_fill_manual(values=palette34)
```
...
```{r Distance0, echo=FALSE}
housesTrain %>% filter(Distance == 0) %>% nrow
```


```{r TransfDistance, message=FALSE}
...
No hay demasiados indicios de que relación lineal entre las coordenadas y el precio de la vivienda.
...
La primera decisión que se toma es eliminar la variables *BuildingArea*. Dicha variable hace referencia al area o tamaño de un espacio, por lo que los valores iguales a cero han de entenderse como datos faltantes. 
...
Aparte de los valores a 0, esta variable presenta valores muy dispersos. Se puede probar una transformación para verla mejor. A la hora de realizar su imputación vamos a ver cómo se relaciona *Landsize* con *Longtitud* , *Lattitude* y *Distance* ya que, pensando en el ámbito en el que estamos trabajando, creemos que la ubicación de la vivienda estará relacionado con el area del perimetro previo a la vivienda.
...
Para las viviendas donde *CouncilArea* no está registrado, lo imputaremos con un KNN utilizando la latitud y longitud.
...
Se categoriza en cuatro grupos de manera que los grupos tienen un número de observaciones más o menos similar. Al verla en relación a *Price*, se observa que los datos faltantes no se distribuyen de manera aleatoria sino que, siguiendo la distribución para los otros factores, se concentran entorno al 1970/1960 aproximadante.
...
year_categoriesImp$Price <- housesTrain$Price

year_categoriesImp %>%
          select(Price, year_cat2) %>%
          ggplot(aes(x=Price, colour=year_cat2)) +
          geom_density()
...
housesTrainFinal %>% select(year_built_cat) %>% head()
```
...
```{r}
housesTrainFinal %>% select(sqrt_distance, log_price, Type) %>% ggpairs(columns=1:3, ggplot2::aes(colour=Type, alpha=0.1))
```
```{r}
...
* Estudiar interacciones entre variables. 
* Generación de nuevas variables a partir de las actuales para ver si correlan en mayor medida con el target.
