...
```{r}
quantiles_price <- quantile(housesTrain$Price)
housesTrain <- housesTrain %>% mutate(price_label = Price > median(Price), price_label_high = Price > quantiles_price[4])
```

```{r}
housesTrain %>% ggplot(aes(x=Price, color=price_label)) + geom_density()
housesTrain %>% ggplot(aes(x=Price, color=price_label_high)) + geom_density()
```
...
numeric_cols <- c("Rooms", "sqrt_distance", "Bedroom2", "Bathroom", "Car", "price_label")

p<-housesTrain %>% select(numeric_cols) %>%
  na.omit() %>%
  ggpairs(columns=1:6, ggplot2::aes(colour=price_label))
...
numeric_cols2 <- c("Landsize", "BuildingArea", "YearBuilt", 'Lattitude', 'Longtitude', "price_label")
...
housesTrain %>% 
  na.omit() %>%
  mutate(car_cat = cut(Car, breaks = c(0,2,20), labels = c("Hasta 1", "Más de 2"), include.lowest = TRUE, right = FALSE)) %>%
  ggplot() + geom_bar(aes(x=car_cat, fill=price_label)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)

housesTrain %>% 
  na.omit() %>%
  mutate(car_cat = cut(Car, breaks = c(0,2,20), labels = c("Hasta 1", "Más de 2"), include.lowest = TRUE, right = FALSE)) %>%
  ggplot() + geom_bar(aes(x=car_cat, fill=price_label), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
...
housesTrainFinal <- data.frame(housesTrainNumNorm[,num_vars], housesTrain[,cat_vars], housesTrain[, c('Price', 'log_price', 'price_label', 'price_label_high')])
