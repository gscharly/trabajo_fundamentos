...
```{r}
housesTrain %>% summarise(median(Price))
```
```{r}
...
housesTrainKmeans['rooms_cat'] <- housesTrain %>% 
  dplyr::select(rooms_cat) %>% 
  factor(levels = c('Pequeñas', 'Medianas', 'Grandes'), ordered = TRUE)

housesTrainKmeans['car_cat'] <-housesTrain %>% 
  dplyr::select(car_cat) %>% 
  factor(levels = c('Pocas_plazas', 'Muchas_plazas'), ordered = TRUE)

housesTrainKmeans['bath_cat'] <-housesTrain %>% 
  dplyr::select(bath_cat) %>% 
  factor(levels = c('Pocos_baños', 'Muchos_baños'), ordered = TRUE)

housesTrainKmeans['bed_cat'] <-housesTrain %>% 
  dplyr::select(bed_cat) %>% 
  factor(levels = c('Pocos_dormitorios', 'Muchos_dormitorios'), ordered = TRUE)

housesTrainKmeans['sell_rate_cat'] <-housesTrain %>% 
  dplyr::select(sell_rate_cat) %>% 
  factor(levels = c('Menos_populares', 'Más_populares'), ordered = TRUE)
...
clusters2=kmeans(nume,centers=2,nstart=25)
...
SSW[1] <- (n - 1) * sum(apply(nume,2,var))
#Variabilidad de cada modelo, desde 2 clusters hasta 15 clusters
for (i in 2:15) SSW[i] <- sum(kmeans(nume,centers=i,nstart=25)$withinss)
...
#Según la gráfica el número de cluster óptimo sería 3 o 4. Empezamos con 3
clusters3=kmeans(nume,centers=3,nstart=25)

#Comprobamos la variabilidad
clusters3$withinss

#Plots de la salida
nk_3=3 # nk es el numero de clusters
pairs(nume,col= clusters3$cluster,pch=19)
points(clusters3$centers, col = 1:nk, pch = 19, cex=2)
clusplot(nume, clusters3$cluster, color=TRUE, shade=TRUE,
labels=2,lines=0)

# Probamos con 4
clusters4=kmeans(nume,centers=4,nstart=25)
...
#Plots de la salida
nk_4= 4
pairs(nume,col= clusters4$cluster,pch=19)
points(clusters4$centers, col = 1:nk, pch = 19, cex=2)
clusplot(nume, clusters4$cluster, color=TRUE, shade=TRUE,
labels=2,lines=0)
```
