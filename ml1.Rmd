...
Aplicamos WOE y Information Value

```{r}
#Seleccionar las variables ya transformadas e imputadas
#También he quitado Date, Postcode, Adress y SellerG (Estas no las usabamos no????)
a <- housesTrain %>% select("CouncilArea", "sqrt_distance_std", "log_landsize_std", "lattitude_std", "longtitude_std", "rooms_cat", "year_built_cat", "car_cat",   "Regionname", "Type", "Method", "bath_cat", "bed_cat", "sell_rate_cat", "price_label_high", "Rooms")

# – Information value: all variables
a$price_exp <- a %>% pull(price_label_high) %>% as.numeric()
a <- a[-15]
table(a$price_exp)

infoTables <- create_infotables(data = a, y = "price_exp",
                              bins = 10,
                              parallel = F)

# – WOE table:
#Se puede mirar para cada variable
infoTables$Tables$Rooms

#Information Value
print(infoTables$Summary, row.names=T)
...
Estas son todas las variables que pueden emplearse para los métodos de clustring. Sin embargo, no se empezará probando con todas ellas sino que se tomarán de menos a más con la intención de encontrar el mejor clustering con el menor número de variables posible. 

```{r}
housesTrainCluster <- housesTrain %>% dplyr::select(sqrt_distance_std, log_landsize_std, lattitude_std, longtitude_std, rooms_cat, car_cat, bath_cat, bed_cat, sell_rate_cat, price_label_high, rooms_cat_woe)
...
housesTrain$rooms_cat_woe <- cut(housesTrain$Rooms,  breaks = c(1,2,3,4,10), labels = c("Pequeñas", "Medianas-Pequeñas", "Medianas-Grandes", "Grandes"), include.lowest = TRUE, right = FALSE)

housesTrainCluster['rooms_cat_woe'] <-housesTrain %>% pull('rooms_cat_woe') %>% 
  ordered(levels = c("Pequeñas", "Medianas-Pequeñas", "Medianas-Grandes", "Grandes")) %>% as.numeric() %>% scale()
```
...
```{r kmeansKoptimo1}
set.seed(123)
selec_var_kmeans <- housesTrainCluster %>% select(sqrt_distance_std)
bss <- kmeans(selec_var_kmeans,centers=1)$betweenss
for (i in 2:15) bss[i] <- kmeans(selec_var_kmeans,centers=i)$betweenss
...
Según los resultados anteriores, el mejor modelo K-means se obtiene utilizando una única variable (distancia al centro) y 5 grupos.
...
```{r hierarchical}
mat_dist <- dist(housesTrainCluster[-10], method = "euclidean")
