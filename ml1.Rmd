...
```{r}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=Regionname, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=Regionname, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
...
```{r TypePrice}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=Type, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=Type, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r MethodPrice}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=Method, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=Method, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=rooms_cat, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=rooms_cat, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=bath_cat, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=bath_cat, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=bed_cat, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=bed_cat, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r}
p1 <- housesTrain %>% ggplot() + geom_bar(aes(x=car_cat, fill=price_label_high)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% ggplot() + geom_bar(aes(x=car_cat, fill=price_label_high), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r, message=FALSE}
p1 <- housesTrain %>% 
  ggplot() + geom_bar(aes(x=sell_rate_cat, fill=price_label)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% 
  ggplot() + geom_bar(aes(x=sell_rate_cat, fill=price_label), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r, message=FALSE}
p1 <- housesTrain %>% 
  ggplot() + geom_bar(aes(x=year_built_cat, fill=price_label)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
p2 <- housesTrain %>% 
  ggplot() + geom_bar(aes(x=year_built_cat, fill=price_label), position='fill') + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values=palette34)
ggpubr::ggarrange(p1, p2, ncol=2, common.legend = TRUE)
```
...
```{r}
p<-housesTrain %>%
  select(Distance, price_label_high) %>%
...
En las siguientes dos gráficas, se muestra el *betweens* y el *withins* para un número de clusters desde 2 hasta 15. Se observa en ambas, que el punto donde se produce el cambio de tendencia es aproximadamente en el 3. Por esto se va a probar a realizar una clasificación en base a 3 grupos. 
...
Observando el ratio de la suma de cuadrados entre-clusters y la suma de cuadrados totales, vemos que el porcentaje de varianza explicada por el modelo respecto al total de varianza observada es inferior al 50%. 
...
housesTrainOrigVar <- housesTrain[c("Rooms", "Type", "Method", "Distance", "Bathroom", "CarImp", "LandsizeImp", "Lattitude", "Longtitude", "Regionname", "year_built_cat", "sell_rate_cat",
"price_label_high")]
...
predictions %>% ggplot(aes(x=pred, fill=price_label_high)) + geom_histogram(alpha=0.5) + scale_fill_manual(values = palette34)
predictions %>% ggplot(aes(x=pred, color=price_label_high)) + geom_density(alpha=0.5) + scale_color_manual(values = palette34)
```
...
DUDA: CURVA ROC EN KNN? PROBABILIDADES?
```{r knn7high}
# En train
prediccion_knn7_train = knn.cv(housesTrain[,num_std_cols], 
                              k=7, cl=housesTrain$price_label_high)
metrics_function(prediccion_knn7_train, housesTrain, 'price_label_high', bool=TRUE)

#En test
prediccion_knn7_test=knn(housesTrain[,num_std_cols], housesTest[,num_std_cols],
                         k=7, cl=housesTrain$price_label_high)

metrics_function(prediccion_knn7_test, housesTest, 'price_label_high', bool=TRUE)
```

### Usando otras variables
Hasta ahora, se han utilizado las 4 variables numéricas que teníamos identificadas. El resto de variables son categóricas, pero algunas además son ordinales, por lo que tendría sentido aplicar distancias utilizando también esas variables.

Además, se puede probar a crear otras variables adicionales como combinaciones de ordinales y numéricas. Por ejemplo, habitaciones por unidad de parcela.

```{r}
housesTrain <- housesTrain %>% mutate(room_land = Rooms/LandsizeImp*1000)
housesTrain %>% filter(room_land<1000) %>% ggplot(aes(y=room_land)) + geom_boxplot()
```


```{r}
housesTrainKnn <- housesTrain %>% dplyr::select(sqrt_distance_std, log_landsize_std, lattitude_std, longtitude_std, rooms_cat, car_cat, bath_cat, bed_cat, sell_rate_cat)
...
```{r}
std_cols <- c('sqrt_distance_std', 'log_landsize_std', 'lattitude_std', 'longtitude_std', 'rooms_cat', 'car_cat', 'bath_cat', 'bed_cat', 'sell_rate_cat')
plot_acc_f1_k(housesTrainKnn, 'price_label_high', std_cols)
```


```{r}
housesTestKnn <- housesTest %>% dplyr::select(sqrt_distance_std, log_landsize_std, lattitude_std, longtitude_std, rooms_cat, car_cat, bath_cat, bed_cat, sell_rate_cat)

#las variables categóricas pasan a ser factores ordenados
housesTestKnn['rooms_cat']<- housesTest %>% pull('rooms_cat') %>% 
  ordered(levels = c('Pequeñas', 'Medianas', 'Grandes')) %>% as.numeric()

housesTestKnn['car_cat'] <- housesTest %>% pull('car_cat') %>% 
  ordered(levels=c("Pocas_plazas","Muchas_plazas")) %>% as.numeric()

housesTestKnn['bath_cat'] <-housesTest %>% pull('bath_cat') %>% 
  ordered(levels = c('Pocos_baños', 'Muchos_baños')) %>% as.numeric()

housesTestKnn['bed_cat'] <-housesTest %>% pull('bed_cat') %>% 
  ordered(levels = c('Pocos_dormitorios', 'Muchos_dormitorios')) %>% as.numeric() 

housesTestKnn['sell_rate_cat'] <-housesTest %>% pull('sell_rate_cat') %>% 
  ordered(levels = c('Menos_populares', 'Más_populares')) %>% as.numeric() 

# Se utiliza el std de train
housesTestKnnNorm <- predict(normParam, housesTestKnn)
housesTestKnnNorm$price_label_high <- as.factor(housesTest$price_label_high)
```

```{r}
#En test
knn_test=knn(housesTrainKnnNorm[,std_cols], housesTestKnnNorm[,std_cols],
                         k=3, cl=housesTrain$price_label_high)

metrics_function(knn_test, housesTestKnnNorm, 'price_label_high', bool=TRUE)
```
