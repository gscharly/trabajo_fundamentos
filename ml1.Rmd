...
Se ha optado por la segunda opción, para introducir una mayor complejidad al problema de clasificación, y porque el corte (en 1.3M) nos parecía lógico.
...
Se rechaza la hipótesis de independencia. El warning que aparece se debe a que dos de nuestras categorías tienen muy pocos ejemplos, y en una parece que no hay ejemplos de casas caras.
```{r, echo=F}
...
Se puede observar que la mayoría de casas caras se engloban en el tipo h, que era el tipo que englobaba casas como fincas, mansiones...
...
Parece que áreas como Bayside o Boroondara tienen casas más caras. Sin embargo, al haber tantas categorías, no es una variable para incluir directamente en los modelos.
...
__year_built_cat__
El problema con esta variable venía con que había muchas casas sin fecha de construcción informada. Sí que se puede intuir que hay más casas caras porcentualmente en la categoría de casas antiguas, pero hay demasiada mezcla en la clase Desconocido como para que sea útil.
...
La correlación más elevada se da para una variable que se ha generado a partir de otra. Observamos que los valores no son demasiado altos, por lo que se podrían utiliza la mayoría de las variables en conjunto en modelos (aunque recordamos que solo se está mirando la correlación lineal).
...
Las casas caras parece que tienen un tamaño de parcela ligeramente mayor. Además la variabilidad respecto al tamaño es menor en las casas caras.
...
__Habitaciones por tamaño de parcela__
Se ha probado a crear esta nueva variable, para combinar la información del número de habitaciones y el tamaño de la parcela. Parece que puede ayudar ligeramente a discriminar las casas.
...
Para seguir con la selección de variables vamos a utilizar el Information Value (IV). Esta medida nos indica cómo de buena es una variable a la hora de distinguir entre las categorías binarias de la variable respuesta o dependiente.

```{r}
a <- housesTrain %>% select("CouncilArea", "sqrt_distance_std", "log_landsize_std", "lattitude_std", "longtitude_std", "rooms_cat", "year_built_cat", "car_cat", "Regionname", "Type", "Method", "bath_cat", "bed_cat", "sell_rate_cat", "price_label_high", "may_have_water")
...
En un primer lugar, los métodos de clustering han sido probados con las variables cuantitativas y cualitativas ordinales del dataset *housesTrain*, todas ellas estandarizadas.

Probando combinaciones de estas variables de menos a más teniendo en cuenta su information value, los mejores resultados en cuanto a menor número de grupos se consiguió utilizando la latitud y el número de baños.

Después de esto, se pasó a utilizar las variables WoE (Weight of Evidence) en relación a la variable dependiente de precio. 

Utilizar estas variables, nos proporciona dos ventajas. La primera es que contamos con variables numéricas que reprensentan el porcentaje de casas caras en cada uno de los bins. La segunda es que había variables categóricas no ordinales que no podíamos emplear en los métodos clustering, pero que presentaban un IV alto, como la variable *type*.  

Los modelos de clustering que se muestran a continuación utilizan las variables WoE.
...
Vamos a buscar el número de clústers óptimo. La idea es que el número seŕa él óptimo cuando los individuos de un mismo grupo sean lo más homogéneos posible y los individuos pertenecientes a distintos grupos sean lo más heterogéneos posible. Esto es lo mismo a buscar una división donde una suma de cuadrados entre (betweens) sea suficientemente grande y, por tanto, una suma de cuadrados dentro lo suficientemente pequeña (withins).
...
De todas las combinaciones probadas teniendo en cuenta la información del apartado de selección de variables y a la forma de la anterior gráfica, vamos a formar los clusters utilizando las variables de latitud y tipo.
...
Observando el ratio de la suma de cuadrados entre-clusters y la suma de cuadrados totales, vemos que el porcentaje de varianza explicada por el modelo respecto al total de varianza observada es muy elevado(86.0%). Aunque este indicador hay que tomarlo con cuidado puesto que se ve influído por el número de clusters empleado de manera que aumenta conforme tomamos más grupos, en este caso solo hemos formado tres clusters. 
...
Ahora, vamos a comprobar si los grupos generados a partir de la combinación de una variable del tamaño de la vivienda (type) y de su localización (lattitud_std) muestran diferencias respecto a la variable target.


```{r}
housesTrain <- housesTrain %>% mutate(kmeans_cluster_woe = clust_kmeans_woe$cluster)
housesTrain$kmeans_cluster_woe <- factor(housesTrain$kmeans_cluster_woe, levels = c(1,2,3))

bar_plot_target(housesTrain, "kmeans_cluster_woe", "price_label_high")
chisq.test(housesTrain$kmeans_cluster_woe, housesTrain$price_label_high)
```
...
Al igual que en kmeans, se van a probar diferentes combinaciones de variables. Sin embargo, es complicado emplear un criterio para seleccionar estas variables puesto que el alto número de observaciones dificulta la legibilidad del dendograma.

Las diferentes combinaciones mostraron que cuando se empleaban tres o más variables la aglomeración de las observaciones se hacía más confusa. También se vio cómo la combinación de dos variables, una de la localización y otra del tamaño, siempre daban lugar a tres grupos muy similares al que se muestra en este dendograma.

```{r hierarchical}
select_var_woe_h = housesTrainWoe %>% select("woe.Regionname.binned","woe.rooms_cat.binned")
mat_dist <- dist(select_var_woe_h, method = "euclidean")
...
avg_col_dend <- color_branches(den2, h = 1, col = palette34)
...
```{r dbscanEpsi}
housesTrainDbscan <- housesTrain %>% select("bath_cat", "lattitude_std")
housesTrainDbscan['bath_cat'] <-housesTrainDbscan %>% pull('bath_cat') %>% 
  ordered(levels = c('Pocos_baños', 'Muchos_baños')) %>% as.numeric() %>% scale()

datos = housesTrainDbscan
dbscan::kNNdistplot(datos, k = 8)
```

La curva tiene el punto de inflexión en torno a 0.1, por lo que se escoge este valor como epsilon para DBSCAN. Se elige además 50 como número mínimo de puntos para formar un cluster.
...
Usamos además las variables categóricas con que implican cierto orden, como factores numéricos ordenados y escalados.
...
De nuevo se representan el precio y la región sobre dos dimensiones.
...
## tSNE
Probamos tsne sobre nuestras variables numéricas escaladas, para que valores extremos no nos den problemas. El parámetro que se suele tocar es el perplexity (aunque hay msá), que es algo parecido al número de vecinos más cercanos cuando se comparan las distribuciones que utiliza tSNE.
Fuente: https://www.analyticsvidhya.com/blog/2017/01/t-sne-implementation-r-python/

De nuevo superponemos distintas variables sobre el gráfico en dos dimensiones, sin poder sacar demasiadas conclusiones.
...
Como primera aproximación, se ha probado el modelo de regresión logística con hasta 12 variables, sabiendo que son demasiadas para un modelo de este tipo. El resultado obtenido es el siguiente:


```{r}
filter_cols <- c('Suburb', 'Address', 'Rooms', 'Distance', 'Lattitude', 'Longtitude', 'SellerG',  'Date', 'Postcode', 'Bedroom2', 'Bathroom', 'YearBuilt', 'CouncilArea', 'CarImp' ,'LandsizeImp', 'Price', 'log_price', 'price_label', "Method", "log_landsize_std", "rooms_cat", "kmeans_cluster")
...
Posteriormente, se han probado diversos modelos con diferentes combinaciones de variables, con el objetivo de obtener un modelo mucho más sencillos con resultados similares al modelo con 11 variables. A continuación se muestra el mejor encontrado:


```{r}
lr_regularized <- readRDS("./models/lr_regularized.rds")
#lr_regularized <- glm(price_label_high ~. -sell_rate_cat - bed_cat - car_cat - may_have_water - lattitude_std - year_built_cat - longtitude_std, family = binomial(link = 'logit'), data = housesTrain_lr)
...
Se puede ver que la f1_score no se ve especialmente perjudicada pasando de 11 a 5 variables. A continuación se muestra por un lado la curva PR (Precision-Recall) junto con el área bajo la misma, y por otro, una comparativa de Precision, Recall y F1 score para los diferentes thresholds:
...
```{r message=FALSE}
model_metrics <- dt_metrics(housesTrain, c("Rooms", "Type", "Method", "Distance", "Bathroom", "CarImp", "LandsizeImp", "Lattitude", "Longtitude", "Regionname", "year_built_cat", "sell_rate_cat", "may_have_water"), "price_label_high")
...
Con 11 hojas, el f1 baja hasta 0.726.
...
### Cross validation
Utilizaremos las variables que mejor resultados en una búsqueda bayesiana de hiperparámetros para obtener el mejor parámetro de complejidad. 

A modo de comentario, tidymodels solo permite optimizar dos métricas de clasificación en optimización de hiperparámetros: accuracy y roc. Vamos a elegir la segunda, intentando separar al máximo las dos clases, aunque lo que se debería hacer sería intentar optimizar una métrica como el AUC PR, más útil para nuestro problema al evaluar la relación entre precision y recall, y tener un problema ligeramente desbalanceado.


```{r}
set.seed(10)
housesTrainCV <- housesTrain %>% select("Rooms", "Lattitude", "Longtitude", "Type", "price_label_high")
...
path = "./models/dt_bayes_search.rds"
...
Las métricas en train son bastante buenas, obteniendo un f1 óptimo de 0.835
...
Como prueba final, elegimos el parámetro de complejidad cp=0.001033845 (aparece en el top de modelos del CV), y probamos sobre train y validación para ver si se produce sobreajuste.
...
A modo de nota final, se han probado las variables generadas (agua y habitaciones/landsize) sin ningún efecto a aparente en estos modelos.
...
Probamos a intentar buscar los mejores parámetros de esta forma, utilizando las mismas variables
...
En train se obtiene una f1 óptima de 0.8:
...
Como simplificación de redes bayesianas, vamos a entrenar un modelo utilizando Naïve Bayes.
...
opt_f1_nv1<- opt_f1_function_v2(prob_values[,2], housesTrain, 'price_label_high')
c(optimal_threshold = opt_f1_nv1$threshold, precision = opt_f1_nv1$precision, recall = opt_f1_nv1$recall, f1_score = opt_f1_nv1$f1_opt)
...
Probamos a reducir el número de variables, utilizando por ejemplo las variables más importantes seleccionadas por el random forest. El modelo empeora muy ligeramente.

```{r}
mod_best <- naiveBayes(price_label_high ~ sqrt_distance_std + Regionname + Type + rooms_cat, data=housesTrain)
...
c(optimal_threshold = opt_f1_nv$threshold, precision = opt_f1_nv$precision, recall = opt_f1_nv$recall, f1_score = opt_f1_nv$f1_opt)
...
El modelo elegido como el mejor aquel que utiliza las variables sqrt_distance_std + Regionname + Type + rooms_cat. No presenta el mejor f1, pero está muy próximo al modelo con mejor f1, y emplea menos variables que los otros modelos probados.
