...
dt_param_final <- select_best(dt_bayes_search, "roc_auc")
dt_wflow_final <- finalize_workflow(dt_workflow, dt_param_final)
dt_wflow_final_fit <- fit(dt_wflow_final, data = housesTrainCV)
```

```{r}
housesTrainCV$.pred <- predict(dt_wflow_final_fit, 
                          new_data = housesTrainCV)$.pred_class
housesTrainCV$.pred_TRUE <- predict(dt_wflow_final_fit, 
                          new_data = housesTrainCV, type='prob')$.pred_TRUE
metrics(housesTrainCV, truth = price_label_high, .pred, .pred_TRUE)
```

```{r message=FALSE}
table(pred = housesTrainCV$.pred, obs = housesTrainCV$price_label_high)
metrics_function(housesTrainCV$.pred, housesTrainCV, 'price_label_high', bool=TRUE)
roc_dt <- roc(housesTrain$price_label_high, housesTrainCV$.pred_TRUE)
ggplotROCCurve(roc_dt)
opt_f1_obj <- opt_f1_function(housesTrainCV$.pred_TRUE, housesTrainCV, 'price_label_high', 0.1)
opt_th <- opt_f1_obj$threshold
opt_f1_obj$f1_opt
opt_f1_obj$p1
opt_f1_obj$p2
```
...
```{r message=FALSE}
eval_val <- evaluate_validation(housesVal, dt_wflow_final_fit, opt_th)
eval_val
```
...
ggplotROCCurve(roc_dt)
opt_f1_obj <- opt_f1_function(housesTrainImpVars$.pred_TRUE, housesTrainImpVars, 'price_label_high', 0.1)
opt_th <- opt_f1_obj$threshold
opt_f1_obj$f1_opt
opt_f1_obj$p1
opt_f1_obj$p2
...
```{r message=FALSE}
eval_val <- evaluate_validation(housesVal, rf_wflow_final_fit, opt_th)
eval_val
```
...
De nuevo, vemos que el modelo sobreajusta en términos de f1 (de 0.93 a 0.81 en validación).
