...
Como se puede observar, solo existe una fuerte correlación lineal entre las variables *log_landsize* y *log_room_land* (variable creada a partir de *Landsize* y *Rooms*).
...
heatmap_corr(housesNum, numeric_cols)
```
...
__log_room_land__
Se ha creado una nueva variable como combinación del número de habitaciones y el tamaño de la parcela. Esta variable representa el número de habitaciones por unidad de medida de parcela. Las distribuciones en función del precio son bastante parecidas, si bien es cierto que la varianza en las casas baratas es mayor.

```{r}
p<-housesTrain %>% mutate(log_room_land = log10(Rooms/LandsizeImp*1000)) %>%
...
Como prueba final, elegimos el parámetro de complejidad cp=0.019616027, y probamos sobre train y validación para ver si se produce sobreajuste.
...
predictions %>% ggplot(aes(x=pred, fill=price_label_high)) + geom_histogram(alpha=0.5) + scale_fill_manual(values = palette34) + geom_vline(xintercept=opt_th_dt, linetype='dashed', color=palette34[4])
predictions %>% ggplot(aes(x=pred, color=price_label_high)) + geom_density(alpha=0.5) + scale_color_manual(values = palette34) + geom_vline(xintercept=opt_th_dt, linetype='dashed', color=palette34[4])
```
...
rf_search <- try_cv_train(path, houses_folds, rf_grid, rf_param)
```
...
rf_wflow_final_fit <- fit(rf_wflow_final, data = housesTrainImpVars)
```

```{r}
housesTrainImpVars$.pred <- predict(rf_wflow_final_fit, 
                          new_data = housesTrainImpVars)$.pred_class
housesTrainImpVars$.pred_TRUE <- predict(rf_wflow_final_fit, 
                          new_data = housesTrainImpVars, type='prob')$.pred_TRUE
metrics(housesTrainImpVars, truth = price_label_high, .pred, .pred_TRUE)
```

Train:

```{r}
table(pred = housesTrainImpVars$.pred, obs = housesTrainImpVars$price_label_high)
metrics_function(housesTrainImpVars$.pred, housesTrainImpVars, 'price_label_high', bool=TRUE)
roc_dt <- roc(housesTrain$price_label_high, housesTrainImpVars$.pred_TRUE)
ggplotROCCurve(roc_dt)
opt_f1_obj <- opt_f1_function(housesTrainImpVars$.pred_TRUE, housesTrainImpVars, 'price_label_high', 0.3)
opt_f1_obj$f1_opt
opt_f1_obj$p1
opt_f1_obj$p2
```
...
```{r}
# Prepare for final prediction
rf_param_final <- select_best(rf_bayes_search, "roc_auc")
rf_wflow_final <- finalize_workflow(rf_workflow, rf_param_final)
rf_wflow_final_fit <- fit(rf_wflow_final, data = housesTrainImpVars)
```
...
table(pred = housesTrainImpVars$.pred, obs = housesTrainImpVars$price_label_high)
metrics_function(housesTrainImpVars$.pred, housesTrainImpVars, 'price_label_high', bool=TRUE)
roc_dt <- roc(housesTrainImpVars$price_label_high, housesTrainImpVars$.pred_TRUE)
ggplotROCCurve(roc_dt)
opt_f1_obj <- opt_f1_function(housesTrainImpVars$.pred_TRUE, housesTrainImpVars, 'price_label_high', 0.3)
opt_f1_obj$f1_opt
opt_f1_obj$p1
...
De nuevo, vemos que el modelo sobreajusta en términos de f1 (de 0.87 a 0.8 en validación).
