...
```{r, echo=F}
saveRDS(lr_model, "./models/lr_all_variables.rds")
...
```{r,echo=F}
saveRDS(lr_regularized, "./models/lr_regularized.rds")
...
```
Como se puede comprobar, sobre todo a partir del área bajo la curva PR, que los resultados obtenidos por este modelo no son demasiado favorables.
...
x_validation <- validation %>% select(-price_label_high)
prob_values_test <- lr_regularized %>% predict(x_validation, type = "response")
