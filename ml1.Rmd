...
train$kmeans_cluster <- factor(train$kmeans_cluster, levels = c(1,2,3,4,5))
train$price_label_high <- as.factor(ifelse(train$price_label_high==TRUE,1,0))

lr_model <- readRDS("./models/lr_all_variables.rds")
#r_model <- glm(price_label_high ~ ., family = binomial(link = 'logit'), data = train)
summary(lr_model)
```
...
```{r}
x_train <- train %>%  select(-price_label_high) 
prob_lr <- lr_model %>% predict(x_train, type = "response")
...
```{r}
prob_lr_regularized <- lr_regularized %>% predict(x_train, type = "response")
...
```{r}
predictions <- data.frame(price_label_high=train$price_label_high, pred=prob_lr_regularized)
...
prob_values_test <- lr_regularized %>% predict(validation, type = "response")
...
housesTrainCV$price_label_high <- factor(housesTrainCV$price_label_high, levels=rev(levels(housesTrainCV$price_label_high)))
...
Como prueba final, elegimos el parámetro de complejidad cp=0.013981636, y probamos sobre train y validación para ver si se produce sobreajuste.

Sobre train, las métricas serán iguales que en la primera prueba que hicimos al podar el árbol.
...
Sobre validación, observamos que las métricas empeoran un poco, aunque el mejor f1 

```{r message=FALSE}
validation_metrics <- val_metrics(pruneTREE, housesVal, c("Rooms", "Lattitude", "Longtitude", "Type"), "price_label_high")
...
```{r}
prob_values <- predict(pruneTREE, housesVal, type='prob')
predictions <- data.frame(price_label_high=housesVal$price_label_high, pred=prob_values[,2])
predictions %>% ggplot(aes(x=pred, fill=price_label_high)) + geom_histogram(alpha=0.5) + scale_fill_manual(values = palette34) + geom_vline(xintercept=validation_metrics$opt_f1$threshold, linetype='dashed', color=palette34[4])
predictions %>% ggplot(aes(x=pred, color=price_label_high)) + geom_density(alpha=0.5) + scale_color_manual(values = palette34) + geom_vline(xintercept=validation_metrics$opt_f1$threshold, linetype='dashed', color=palette34[4])
```
...
Además, se puede probar a utilizar las variables transformadas
...
roc_dt <- roc(housesTrain$price_label_high, housesTrainOrigVar$.pred_TRUE)
ggplotROCCurve(roc_dt)
...
```{r message=FALSE}
housesValRF <- housesVal
housesValRF$price_label_high <- as.factor(housesValRF$price_label_high)
housesValRF$.pred <- predict(rf_wflow_final_fit, 
                          new_data = housesValRF)$.pred_class
housesValRF$.pred_TRUE <- predict(rf_wflow_final_fit, 
                          new_data = housesValRF, type='prob')$.pred_TRUE
metrics(housesValRF, truth = price_label_high, .pred, .pred_TRUE)
table(pred = housesValRF$.pred, obs = housesValRF$price_label_high)
metrics_function(housesValRF$.pred, housesValRF, 'price_label_high', bool=TRUE)
roc_dt <- roc(housesValRF$price_label_high, housesValRF$.pred_TRUE)
ggplotROCCurve(roc_dt)
opt_f1_obj <- opt_f1_function(housesValRF$.pred_TRUE, housesValRF, 'price_label_high', 0.3)
opt_f1_obj$f1_opt
opt_f1_obj$p1
opt_f1_obj$p2
```
...
Probamos un modelo con unos parámetros más simples (de los que aparecen en el top10 del cv):
...
```{r message=FALSE}
validation_metrics <- val_metrics(rf4, housesVal, c("Rooms", "Distance", "LandsizeImp",
...
metrics_function_num(factor(final_preds_val), housesVal, "price_label_high")
...
metrics_function_num(preds_svm_val, validation_svm, 'price_label_high')
```
...
```{r}
mod <- naiveBayes(price_label_high ~ Distance + Regionname + Type + rooms_cat + kmeans_cluster, data=housesTrain)
```
...
Ahora probamos el modelo con menos variables sobre los datos de validación. El F1 obtenido es ligeramente inferior al obtenido en train (0,68) y una auc roc de 0,897.

Finalmente, buscamos el corte óptimo con el mejor modelo.

```{r message=FALSE}
mod <- naiveBayes(price_label_high ~ Distance + Regionname + Type + rooms_cat + kmeans_cluster, data=housesVal)

prob_values <- predict(mod, housesVal, type='raw')
roc_dt <- roc(housesVal$price_label_high, prob_values[,2])
ggplotROCCurve(roc_dt)

opt_f1_function_v2(prob_values, housesVal, 'price_label_high')
...
saveRDS(mod, "./models/nb_best_model.rds")
```
...
values <- predict(model_lr, housesVal, type='response')
f1_lr = round(opt_f1_function_v2(values, data_lr, 'price_label_high')$f1_opt, 3)

#Árbol de decision
data_dt = housesVal
model_dt = readRDS("./models/dt_best_model.rds")
values = predict(model_dt, housesVal, type='prob')
f1_dt = round(opt_f1_function(values, data_dt, 'price_label_high')$f1_opt, 3)

#Random forest
data_rf = housesVal
model_rf = readRDS("./models/rf_best_model.rds")
values = predict(model_rf, housesVal, type='prob')
f1_dt = round(opt_f1_function(values, data_dt, 'price_label_high')$f1_opt, 3)

#SWM
data_svm = validation_svm
model_svm = readRDS("./models/svm_gaussian_kernel.rds")
values <- predict(model_svm, housesVal, type = "prob")
f1_svm = round(opt_f1_function(values, data_svm, "price_label_high")$f1_opt, 3)
f1_svm

#Naive Bayes
data_nv = housesVal
model_nv = readRDS("./models/nb_best_model.rds")
values <- predict(model_nv, housesVal, type='raw')
f1_nv = round(opt_f1_function(values, data_nv, 'price_label_high')$f1_opt, 3)


dfMetricas <- data.frame(
  Modelos = c('Regresión logística', 'Árbol de decisión', 'Random forest', 'KNN', 'Naive Bayes'),
  F1 = c(f1_lr, f1_dt, 'x', 'x', f1_nv))

dfMetricas %>% kable() %>% kable_styling(bootstrap_options = c('hover'), position = 'center') %>%  row_spec(2, bold = T, color = "white", background = palette34[1])
